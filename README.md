...
> - **🌴 CLOUD Branding + Tropical Blues**: Professional SynthScan-style branding with large "CLOUD" hero title, "Awareness Bridge Router • FractiAI" subtitle, and warm tropical blue color palette (replacing bright cyan). Professional 3-tier header hierarchy with ONLINE status badge, Frontier HHF-AI tagline, and "New Transmission" button.
> - **📦 Collapsible Sidebar**: Cloud Channel now collapses from 320px to 48px with smooth transition, tropical blue button on left edge, ChevronLeft/Right icons. User-controlled for space management.
> - **🎯 UI Refinements**: Removed FractiAI Research Team indicator from Creator Dashboard for cleaner UI. Moved BETA ACTIVE badge to far right of status bar with `ml-auto` for better visual hierarchy.
> - **🔧 TypeScript Fixes**: Added `redundancy_penalty_percent_computed` and `sweet_spot_bonus_multiplier_computed` to evaluation interfaces, fixing CI/CD errors from Marek/Simba transparency enhancement.
> - **🌀 Fractal Nodes**: Posts styled as self-similar fractal nodes with 3-layer structure, conic gradient rotation, scanning beams, corner pulses, and hydrogen spectrum color coding by role/status.
