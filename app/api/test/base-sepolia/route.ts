...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        try {
            ...
            if (balance < minBalance) {
                ...
                results.warnings.push('Balance is low. Get Sepolia ETH from: https://www.coinbase.com/faucets/base-ethereum-goerli-faucet')
            }
        } catch (error) {
            results.tests.walletBalance = {
                status: 'FAIL',
        ...
        if (synth90TAddress) {
            try {
                ...
                results.tests.synth90TContract = {
                    ...
                    note: code === '0x' ? 'Contract not deployed on Sepolia. This is expected if using mainnet addresses.' : 'Contract found on Sepolia'
                }
            } catch (error) {
                results.tests.synth90TContract = {
                    status: 'ERROR',
        ...
        if (!hasErrors && results.tests.walletBalance?.hasSufficientBalance) {
            ...
            results.summary.message = '⚠️ Network connected but balance is low. Get Sepolia ETH from faucet.'
        }
    ...
    const statusCode = results.summary.status === 'ERROR' ? 500 : 
                      results.summary.status === 'SUCCESS' ? 200 : 400
